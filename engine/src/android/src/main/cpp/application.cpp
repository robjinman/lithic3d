...
int32_t EventHandler::onInputEvent(const AInputEvent& event)
{
  if (m_app) {
    ...
    if (eventType == AINPUT_EVENT_TYPE_KEY) {
      ...
      m_logger.debug(STR("Input event, key = " << key));
...
{
  lithic3d::autoHideNavBar(state);

  auto logger = lithic3d::createAndroidLogger();
  logger->info("Starting Freehold Game Engine Application");

  lithic3d::EventHandler handler{*logger};

  state->onAppCmd = lithic3d::handleCommand;
  state->onInputEvent = lithic3d::handleInput;
  state->userData = &handler;

  if (!lithic3d::waitForWindow(*state, handler)) {
    return;
  }
