...
template<>
struct std::hash<fge::render::MeshFeatureSet>
{
  size_t operator()(const fge::render::MeshFeatureSet& x) const noexcept
  {
    return fge::hashAll(
      x.vertexLayout,
      x.flags
...
template<>
struct std::hash<fge::render::MaterialFeatureSet>
{
  size_t operator()(const fge::render::MaterialFeatureSet& x) const noexcept
  {
    return fge::hashAll(
      x.flags
    );
